...
import com.mozhimen.basick.utilk.squareup.moshi.toTMoshi
    ...
    override fun initView(savedInstanceState: Bundle?) {
        val activityResultLauncher1 = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode == Activity.RESULT_OK) {
                val hmsScan: HmsScan? = result.data?.getStringExtra(ScanKQRActivity.SCANK_ACTIVITY_RESULT_PARAM)?.toTMoshi<HmsScan>() ?: kotlin.run {
        ...
        val activityResultLauncher2 = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
            if (result.resultCode == Activity.RESULT_OK) {
                val scanKQR2Result: ScanKQR2Activity.ScanK2Result? =
                    result.data?.getStringExtra(ScanKQR2Activity.SCANK2_ACTIVITY_RESULT_PARAM)?.toTMoshi<ScanKQR2Activity.ScanK2Result>() ?: kotlin.run {
